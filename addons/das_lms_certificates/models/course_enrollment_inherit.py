...
class CourseEnrollmentInherit(models.Model):
    ...
    das_lms_final_status = fields.Selection(
        [("pending", "Pendiente"), ("approved", "Aprobado"), ("failed", "Reprobado")],
        string="Estado Final de Certificación",
        default="pending",
    )

    das_lms_survey_completed = fields.Boolean(
        string="Encuesta de Satisfacción Completada", default=False
    )
    ...
    das_lms_certificate_date = fields.Date(
        string="Fecha del Certificado", compute="_compute_certificate_date", store=True
    )

    das_lms_certificate_qr = fields.Binary(
        string="Código QR", compute="_compute_certificate_qr"
    )
    ...
    def _compute_certificate_qr(self):
        for rec in self:
            if rec.das_lms_certificate_token:
                url = "%s/validar/certificado/%s" % (
                    rec.get_base_url(),
                    rec.das_lms_certificate_token,
                )
                try:
                    barcode = rec.env["ir.actions.report"].barcode(
                        "QR", url, width=100, height=100
                    )
    ...
    def _compute_certificate_date(self):
        for rec in self:
            if (
                rec.das_lms_final_status in ["approved", "failed"]
                and not rec.das_lms_certificate_date
            ):
    ...
    def action_generate_certificate(self):
        ...
        # 🔒 VALIDACIÓN 1: Examen obligatorio
        if self.das_lms_final_status == "pending":
            raise UserError(
                "Debes completar el examen final antes de generar el certificado."
            )

        # 🔒 VALIDACIÓN 2: Encuesta obligatoria (CRÍTICA)
        if not self.das_lms_survey_completed:
            raise UserError(
                "Debes completar la encuesta de satisfacción antes de generar el certificado."
            )
