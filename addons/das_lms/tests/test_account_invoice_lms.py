...
class TestDasLmsAccountInvoiceEnrollment(AccountTestInvoicingCommon):
    ...
    def test_customer_invoice_post_enrolls_and_blocks_repurchase(self):
        ...
        msg = tmpl._das_lms_cart_validation_message(partner, new_qty=1, product_product=product)
        self.assertTrue(msg)

    def test_customer_invoice_post_enrolls_multiple_distinct_courses(self):
        partner = self.partner_a
        product_a = self.env['product.product'].create({
            'name': 'Curso LMS factura A',
            'lst_price': 40.0,
            'sale_ok': True,
            'type': 'service',
            'invoice_policy': 'order',
        ...
        self.env['slide.channel'].create({
            'name': 'Canal LMS factura A',
            'product_id': product_a.id,
            'enroll': 'payment',
        })
        self.env['slide.channel'].create({
            'name': 'Canal LMS factura B',
            'product_id': product_b.id,
            'enroll': 'payment',
        })
        tmpl_a = product_a.product_tmpl_id
        tmpl_b = product_b.product_tmpl_id
        invoice = self.env['account.move'].create({
            'move_type': 'out_invoice',
            'partner_id': partner.id,
            'invoice_date': fields.Date.context_today(self.env.user),
        ...
        invoice.action_post()
        self.assertTrue(tmpl_a._das_lms_user_is_enrolled(partner=partner))
        self.assertTrue(tmpl_b._das_lms_user_is_enrolled(partner=partner))
    ...
    def test_invoice_line_without_product_id_uses_sale_line_link(self):
        ...
        invoice.action_post()
        self.assertTrue(tmpl._das_lms_user_is_enrolled(partner=partner))
        self.assertTrue(tmpl._das_lms_cart_validation_message(partner, new_qty=1, product_product=product))
